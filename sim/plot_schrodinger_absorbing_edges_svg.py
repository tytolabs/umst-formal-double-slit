...
"""
Stdlib SVG: |psi|^2 vs x from `schrodinger_1d_absorbing_edges.py` output
(`schrodinger_absorbing_edges_rho.csv`, columns `x`, `rho`).
"""
...
def _default_out() -> Path:
    return Path(__file__).resolve().parent / "out" / "schrodinger_absorbing_edges.svg"


def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="SVG for absorbing-edges rho vs x.")
    p.add_argument(
        "--in",
        dest="in_path",
        type=str,
        default=None,
        help="CSV x,rho (default: sim/out/schrodinger_absorbing_edges_rho.csv)",
    )
    p.add_argument(
        "--out",
        type=str,
        default=None,
        help="Output SVG (default: sim/out/schrodinger_absorbing_edges.svg)",
    )
    p.add_argument(
        "--validate",
        action="store_true",
...
def build_svg(xs: List[float], rs: List[float], width: int = 680, height: int = 400) -> str:
    ...
    parts: List[str] = [
        ...
        (
            ...
            f"1D absorbing edges: |psi|^2 vs x</text>"
        ),
...
def main() -> None:
    args = _parse_args()
    default_csv = Path(__file__).resolve().parent / "out" / "schrodinger_absorbing_edges_rho.csv"
    in_path = Path(args.in_path) if args.in_path else default_csv
    out_path = Path(args.out) if args.out else _default_out()
    if not in_path.is_file():
        raise SystemExit(f"missing {in_path} — run schrodinger_1d_absorbing_edges.py first")
