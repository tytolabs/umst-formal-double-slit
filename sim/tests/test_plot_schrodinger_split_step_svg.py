...
PLOT = ROOT / "sim" / "plot_schrodinger_split_step_svg.py"


class PlotSchrodingerSplitStepSvgTests(unittest.TestCase):
    def test_plot_from_minimal_csv(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with csv_path.open("w", encoding="utf-8", newline="") as f:
                f.write("x,rho\n")
                for i in range(16):
                    f.write(f"{-3.0 + 0.4 * i},{0.02 + 0.002 * (i % 5)}\n")
            subprocess.run(
            ...
            self.assertIn("split-step", text)

    def test_plot_from_repo_csv_if_present(self):
        csv_path = ROOT / "sim" / "out" / "schrodinger_split_step_rho.csv"
        if not csv_path.is_file():
            self.skipTest("run schrodinger_1d_split_step.py first")
        with tempfile.TemporaryDirectory() as tmp:
