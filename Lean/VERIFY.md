...
**Sorry:** **none** in any Lean file — **0 `sorry`** across the entire project. **Phase 5 (information theory):** **`VonNeumannEntropy.lean`** — general `Fin n` **`vonNeumannEntropy_unitarily_invariant`** is **proved** (via `charpoly` + eigenvalue multiset). **`DataProcessingInequality.lean`** — general unital DPI / **Klein** stated as **axiom** (requires Mathlib matrix logarithm infrastructure); identity channel + qubit DPI **proved**. **Tier 1b** (qubit diagonal ≥ spectral) is **proved** (`vonNeumannDiagonal_ge_vonNeumannEntropy`). **`GeneralVisibility.lean`** — `fringeVisibility_n_le_one` **proved** (no axiom). **`LindbladDynamics.lean`** — `dephasingSolution_tendsto_diagonal` **proved** (no axiom). **4 axioms** (line-start scan): `klein_inequality`, `vonNeumannEntropy_nondecreasing_unital` (DPI), `physicalSecondLaw` (Landauer), `vonNeumannEntropy_tensorDensity` (product-state entropy additivity — Kronecker spectrum gap in Mathlib). See `../Docs/SORRY_ROADMAP.md`.
...
**Stats (heuristic):** from repo root, `make lean-stats` / `make lean-stats-md` → `scripts/lean_decl_stats.py`. *Last pasted to `PROOF-STATUS.md`:* **512** `theorem`, **60** `lemma`, **4** `axiom`, **58** `.lean` files (line-start scan, not a full parser).
