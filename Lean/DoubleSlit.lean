...
/-!
# DoubleSlit — full-chain import closure

Import this module (or run `lake build`) to ensure **complementarity**, **Landauer diagonal costing**,
**gate compatibility**, and **which-path measurement update** layers all compile together.

**`measurementUpdateWhichPath`** and related lemmas live in `WhichPathMeasurementUpdate.lean` (same
`UMST.DoubleSlit` namespace) so `EpistemicSensing` can import them without creating an import cycle
through `GateCompat`.
