...
7. **Probe-strength cost hook**: `LandauerCostFromProbeStrength` uses `ProbeStrength` (often distinguishability-based) as a bit-equivalent surrogate for optimization/selection; it is a formal utility/cost hook, not a derived thermodynamic equality.
6. **`thermoFromQubitPath`**: minimal **scaffold** mapping Born weights into `ThermodynamicState`; not a materials or continuum model.
