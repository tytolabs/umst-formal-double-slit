...
\textit{Machine-checked in Lean\,4 with Mathlib.\; \textbf{533 theorems + 34 lemmas} in 58 \texttt{lakefile} roots (\textbf{542+35} over all \texttt{Lean/*.lean}), \textbf{0} tactic \texttt{sorry}, \textbf{1} explicit \texttt{axiom} (\texttt{physicalSecondLaw}); visibility bound and dephasing diagonal limit are \textbf{theorems}; Klein spectral relative entropy $\geq 0$ is proved.}
\end{leftbox}
...
\midrule
Formal proof & Lean 4 + Mathlib & 58 roots, 533 thm., 34 lem. (542+35 all \texttt{Lean}) & \textbf{0 sorry}, \textbf{1 axiom} \\
...
%--- Our work ---
\bibitem{umst-formal} S.~Shyamsundar and S.\,P.~Shenbagamoorthy, ``Formal Verification and Categorical Semantics for the UMST/DUMSTO Thermodynamic Gate,'' \texttt{umst-formal} (double-slit extension track), GitHub, 2026. Lean\,4: 533 \texttt{theorem}s + 34 \texttt{lemma}s in 58 \texttt{lakefile} roots (542+35 over all \texttt{Lean/*.lean}); 68 \texttt{.lean} files; 0 tactic \texttt{sorry}; 1 \texttt{axiom} (\texttt{physicalSecondLaw}) — see \texttt{FORMAL\_FOUNDATIONS.md} / \texttt{PROOF-STATUS.md}.
