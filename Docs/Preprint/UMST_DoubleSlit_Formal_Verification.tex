...
\vspace{0.3em}
{\small\color{MidGray}%
\textbf{Santhosh Shyamsundar} --- Studio TYTO\quad\texttt{santhoshshyamsundar@tyto.studio}\\[2pt]
\textbf{Santosh Prabhu Shenbagamoorthy} --- Studio TYTO\quad\texttt{santosh@tyto.studio}\\[3pt]
...
\textit{Machine-checked in Lean\,4 with Mathlib.\; \textbf{537 theorems + 34 lemmas} in 59 \texttt{lakefile} roots (\textbf{546+35} over all \texttt{Lean/*.lean}; script excludes \texttt{.lake}), \textbf{0} tactic \texttt{sorry}, \textbf{1} explicit \texttt{axiom} (\texttt{physicalSecondLaw}); visibility bound, dephasing diagonal limit, and discrete stream-D limit are \textbf{theorems}; Klein spectral relative entropy $\geq 0$ is proved.}
\end{leftbox}
...
\midrule
Formal proof & Lean 4 + Mathlib & 59 roots, 537 thm., 34 lem. (546+35 all \texttt{Lean}) & \textbf{0 sorry}, \textbf{1 axiom} \\
...
%--- Our work ---
\bibitem{umst-formal} S.~Shyamsundar and S.\,P.~Shenbagamoorthy, ``Formal Verification and Categorical Semantics for the UMST/DUMSTO Thermodynamic Gate,'' \texttt{umst-formal} (double-slit extension track), GitHub, 2026. Lean\,4: 537 \texttt{theorem}s + 34 \texttt{lemma}s in 59 \texttt{lakefile} roots (546+35 over all \texttt{Lean/*.lean}); 0 tactic \texttt{sorry}; 1 \texttt{axiom} (\texttt{physicalSecondLaw}) — see \texttt{FORMAL\_FOUNDATIONS.md} / \texttt{PROOF-STATUS.md}.
