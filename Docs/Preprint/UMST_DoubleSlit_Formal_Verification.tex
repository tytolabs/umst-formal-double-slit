...
\vspace{0.3em}
{\small\color{MidGray}%
\textbf{Santhosh Shyamsundar} --- Studio TYTO; IAAC Barcelona\quad\texttt{santhoshshyamsundar@studio.tyto}\\[2pt]
\textbf{Santosh Prabhu Shenbagamoorthy} --- Studio TYTO; IAAC Barcelona\quad\texttt{santosh@studio.tyto}\\[3pt]
...
\vspace{0.6em}
\begin{center}
\includegraphics[width=0.95\textwidth]{../Media/teaser.png}
\end{center}
\vspace{0.4em}
...
\textit{Machine-checked in Lean\,4 with Mathlib.\; \textbf{467 theorems, 0 \texttt{sorry}, 5 explicit axioms} (corresponding to physical principles not yet formalized in Mathlib).}
\end{leftbox}
...
\]
This is the content of the Lean theorem \texttt{complementarity\_fringe\_path} in module \texttt{QuantumClassicalBridge}. Any gain in which-path distinguishability $I$ mathematically necessitates a rigid destruction of interference visibility $V$.
...
\item \textbf{Landauer layer:} \texttt{landauerCostDiagonal} is a scale $k_B T \ln 2 \times (\text{diagonal entropy in bit-equivalents})$---not a claim that measured heat equals this without an explicit erasure/bath model.
\end{enumerate}
...
\item Quantum DPI / mutual information on compound systems (listed as future in \texttt{InfoEntropy}).
...
\midrule
Formal proof & Lean 4 + Mathlib & 48 modules, 467 theorems & \textbf{0 sorry}, \textbf{5 axioms} \\
...
\noindent{\small\textit{Repository:} \texttt{github.com/tytolabs/umst-formal-double-slit} \quad\textbar\quad \textit{License:} MIT \quad\textbar\quad \textit{Build:} \texttt{cd Lean \&\& lake build}}
...
Portions of this work were developed in collaboration with advanced large-language-model tools. Claude Opus and Sonnet 4.6 (Anthropic) provided surgical precision during drafting and refinement. Gemini 3.1 Pro High (Google) offered exceptional large-context planning and file management. Grok 4.20 (xAI) and its collaborative reasoning team contributed core mathematical and scientific reasoning. The Cursor code editor, Claude Code (Anthropic), and Antigravity (Google) supported seamless implementation.
...
This work relies on the following open-source tools and languages: Lean\,4 (theorem prover), Mathlib (mathematical library), Coq/Rocq (proof assistant), Agda (dependently typed language), Haskell (property testing via QuickCheck), Python (numerical simulation), QuTiP (quantum toolkit), \LaTeX{} (typesetting), Git/GitHub (version control and CI), and GitHub Actions (continuous integration).}
...
%--- Our work ---
\bibitem{umst-formal} S.~Shyamsundar and S.\,P.~Shenbagamoorthy, ``Formal Verification and Categorical Semantics for the UMST/DUMSTO Thermodynamic Gate,'' \texttt{umst-formal} (double-slit extension track), GitHub, 2026. Lean\,4: 467 \texttt{theorem}s, 48 \texttt{lakefile} roots, 54 \texttt{.lean} files; 0 \texttt{sorry}; 5 \texttt{axiom}s (see \texttt{PROOF-STATUS.md}).
